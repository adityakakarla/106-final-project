<script lang='ts'>
    ...
    // Student grades lookup - now labeled as final exam percentages
    const studentGrades = {
        'S1': 91,
        'S2': 90,
        'S3': 94,
    ...
    function createChart() {
        ...
        legend.append('text')
            ...
            .attr('font-size', '18px')
            .attr('font-weight', 'bold')
            .text('Students (Final Exam %)');
            
        // Add legend items
        selectedStudents.forEach((student, i) => {
            const grade = studentGrades[student as keyof typeof studentGrades] || '-';
            ...
            // Add student label with grade percentage
            ...
            .on('mousemove', function(event) {
                ...
                studentData.forEach((data, student) => {
                    ...
                    if (index > 0 && index < data.length) {
                        ...
                        // Add to tooltip content with grade percentage
                        const grade = studentGrades[student as keyof typeof studentGrades] || '-';
                        tooltipContent += `<span style="color:${colorScale(student)}">●</span> ${student} (${grade}%): ${d.value} BPM<br />`;
                    }
                });
...
<div class='h-[100svh] w-full flex flex-col items-center justify-center'>
    <div class='w-[90vw] max-w-[1800px] pb-40 flex flex-col gap-4 bg-white p-8 rounded-xl border-8 border-red-500'>
            ...
            <div class="flex flex-wrap items-center gap-2 justify-center">
                ...
                <div class="flex flex-wrap gap-2 max-w-[1000px]">
                    {#each sortStudentIds([...new Set(hr_data.map((d: HeartRateData) => d.student))] as string[]) as student}
                        ...
                        >
                            {student} ({studentGrades[student as keyof typeof studentGrades] || '-'}%)
                        </button>
                    {/each}
                </div>
        ...
        <div class="flex flex-wrap gap-6 justify-center">
            <button 
                class="flex items-center gap-1 px-6 py-2 bg-red-500 text-white text-xl rounded-md shadow hover:bg-red-600 transition-colors"
