<script lang="ts">
    ...
    let customHeartRate = $state('100'); // New state for input field
    
    // Handle custom heart rate input
    function handleCustomHeartRateChange(event: Event) {
        const input = event.target as HTMLInputElement;
        const value = parseInt(input.value);
        if (!isNaN(value) && value >= 40 && value <= 200) {
            customHeartRate = input.value;
            heartRate = value;
        }
    }
    ...
    function adjustHeartRate(amount: number): void {
        const newRate = Math.max(40, Math.min(200, heartRate + amount));
        heartRate = newRate;
        customHeartRate = newRate.toString();
    }
            ...
            <div class="monitor-display">
                <div class="monitor-header">
                    ...
                    <div class="monitor-controls">
                        <input 
                            type="number" 
                            class="heart-rate-input" 
                            value={customHeartRate}
                            min="40"
                            max="200"
                            on:input={handleCustomHeartRateChange}
                ...
                <div class="settings-panel">
                    ...
                    <div class="settings-controls">
                        <div class="control-group">
                            ...
                            <div class="presets">
                                <button on:click={() => { heartRate = 60; customHeartRate = "60"; }}>Rest (60)</button>
                                <button on:click={() => { heartRate = 120; customHeartRate = "120"; }}>Exercise (120)</button>
                                <button on:click={() => { heartRate = 180; customHeartRate = "180"; }}>Critical (180)</button>
                            </div>
                        </div>
                        <div class="control-group">
